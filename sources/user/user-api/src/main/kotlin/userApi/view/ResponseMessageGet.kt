...
suspend fun RoutingContext.noJwtToken() {
    ...
    globalLogger.atInfo { message = "no [jwtToken] parameter found" }
}
...
suspend inline fun RoutingContext.jwtTokenIsNotValid() {
    ...
    globalLogger.atInfo { message = "[jwtToken] parameter is not valid" }
}

suspend fun RoutingContext.noLogin() {
    call.respond(HttpStatusCode.BadRequest.description("no [login] parameter found"))
    globalLogger.atInfo { message = "no [login] parameter found" }
}
...
suspend inline fun RoutingContext.internalServerError() {
    ...
    globalLogger.atError {
        ...
        payload = buildMap {
            put("route", this@internalServerError.call.route)
            put("query parameters", this@internalServerError.call.queryParameters)
        }
    }
}
